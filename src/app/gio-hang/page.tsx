/* ===================================================================
   PAGE: TRANG GIỏ HÀNG (CART PAGE)
   - Hiển thị danh sách vé tàu đã chọn
   - Tính tổng tiền tự động
   - Empty state nếu chưa có vé
   - Xóa từng vé hoặc xóa tất cả
...
import { useCart } from '@/context/CartContext';  // Hook quản lý giỏ hàng
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function CartPage() {
    // ===== CONTEXT: LẤY DỮ LIỆU GIỏ HÀNG =====
    const { cartItems, removeFromCart, clearCart } = useCart();

    // ===== CALCULATION: TÍNH TỔNG TIỀN =====
    // Reduce: cộng dồn giá của tất cả items trong giỏ hàng
    ...
    if (cartItems.length === 0) {
        return (
            <div className="min-h-screen flex flex-col">
                ...
                {/* Empty state container - center align */}
                <div className="flex-grow bg-[#E6F2FF] py-12 flex flex-col items-center justify-center">
    ...
    // ===== MAIN RENDER: HIỂN THỊ CART CÓ VÉ =====
    return (
        <div className="min-h-screen flex flex-col">
            <Header />
            ...
            <main className="flex-grow bg-[#E6F2FF] py-8">
                <div className="container mx-auto px-4 max-w-5xl">
                    ...
                    {/* ===== PAGE HEADER - TIÊU ĐỀ VÀ NÚT XÓA TẤT CẢ ===== */}
                    ...
                    {/* ===== GRID LAYOUT: CART ITEMS + SUMMARY SIDEBAR =====
                        - 1 cột mobile, 3 cột desktop (items 2, summary 1) */}
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

                        {/* ===== CART ITEMS LIST ===== */}
                        <div className="lg:col-span-2 space-y-4">
                            {/* Map qua từng item trong giỏ hàng */}
                            {cartItems.map((item) => (
